...
module Fluent
  class Log
    ...
    def self.str_to_level(log_level_str)
      puts 'jinchen ----- log.rb : self.str_to_level start'
      ...
      puts 'jinchen ----- log.rb : self.str_to_level end'
    end

    def initialize(out=STDERR, level=LEVEL_TRACE, opts={})
      puts 'jinchen ----- log.rb : initialize start'
      @out = out
      @level = level
      @debug_mode = false
      @self_event = false
      ...
      puts 'jinchen ----- log.rb : initialize end'
    end
    ...
    def enable_debug(b=true)
      puts 'jinchen ----- log.rb : enable_debug'
      @debug_mode = b
      self
    end

    def enable_event(b=true)
      puts 'jinchen ----- log.rb : enable_event'
      @self_event = b
      self
    end

    def enable_color?
      puts 'jinchen ----- log.rb : enable_color?'
      !@color_reset.empty?
    end
    ...
    def trace_backtrace(backtrace=$!.backtrace)
      puts 'jinchen ----- log.rb : trace_backtrace'
      dump_stacktrace(backtrace, LEVEL_TRACE)
    end

    def on_debug(&block)
      puts 'jinchen ----- log.rb : on_debug'
      return if @level > LEVEL_DEBUG
      block.call if block
    end

    def debug(*args, &block)
      puts 'jinchen ----- log.rb : debug'
      return if @level > LEVEL_DEBUG
      args << block.call if block
    ...
    def debug_backtrace(backtrace=$!.backtrace)
      puts 'jinchen ----- log.rb : debug_backtrace'
      dump_stacktrace(backtrace, LEVEL_DEBUG)
    end

    def on_info(&block)
      puts 'jinchen ----- log.rb : on_info'
      return if @level > LEVEL_INFO
      block.call if block
    end

    def info(*args, &block)
      puts 'jinchen ----- log.rb : info'
      return if @level > LEVEL_INFO
      args << block.call if block
      time, msg = event(:info, args)
      jinchen = 'jinchen' 
      puts [@color_info, caller_line(time, @depth_offset, LEVEL_INFO), msg + jinchen, @color_reset].join
    rescue
    end
    alias INFO info

    def info_backtrace(backtrace=$!.backtrace)
      puts 'jinchen ----- log.rb : info_backtrace'
      dump_stacktrace(backtrace, LEVEL_INFO)
    end

    def on_warn(&block)
      puts 'jinchen ----- log.rb : on_warn'
      return if @level > LEVEL_WARN
      block.call if block
    end

    def warn(*args, &block)
      puts 'jinchen ----- log.rb : warn'
      return if @level > LEVEL_WARN
      args << block.call if block
    ...
    def warn_backtrace(backtrace=$!.backtrace)
      puts 'jinchen ----- log.rb : warn_backtrace'
      dump_stacktrace(backtrace, LEVEL_WARN)
    end

    def on_error(&block)
      puts 'jinchen ----- log.rb : on_error'
      return if @level > LEVEL_ERROR
      block.call if block
    end

    def error(*args, &block)
      puts 'jinchen ----- log.rb : error'
      return if @level > LEVEL_ERROR
      args << block.call if block
    ...
    def error_backtrace(backtrace=$!.backtrace)
      puts 'jinchen ----- log.rb : error_backtrace'
      dump_stacktrace(backtrace, LEVEL_ERROR)
    end

    def on_fatal(&block)
      puts 'jinchen ----- log.rb : on_fatal'
      return if @level > LEVEL_FATAL
      block.call if block
    end

    def fatal(*args, &block)
      puts 'jinchen ----- log.rb : fatal'
      return if @level > LEVEL_FATAL
      args << block.call if block
    ...
    def fatal_backtrace(backtrace=$!.backtrace)
      puts 'jinchen ----- log.rb : fatal_backtrace'
    ...
    def write(data)
      puts 'jinchen ----- log.rb : write'
      @out.write(data)
    end

    def flush
      puts 'jinchen ----- log.rb : flush'
      @out.flush
    end
    ...
    def dump_stacktrace(backtrace, level)
      puts 'jinchen ----- log.rb : dump_stacktrace'
    ...
    def caller_line(time, depth, level)
      puts 'jinchen ----- log.rb : caller_line'
  ...
  class PluginLogger < Log
    def initialize(logger)
      puts 'jinchen ----- log.rb : plugin-logger : initialize'
      @logger = logger
      @level = @logger.level
      @depth_offset = 3
    ...
    def level=(log_level_str)
      puts 'jinchen ----- log.rb : plugin-logger : level='
    ...
    def enable_color(b = true)
      puts 'jinchen ----- log.rb : plugin-logger : evable_color'
      orig_enable_color b
  ...
  module PluginLoggerMixin
    ...
    def initialize
      puts 'jinchen ----- log.rb : plugin-logger-mixin : initialize'
      super
