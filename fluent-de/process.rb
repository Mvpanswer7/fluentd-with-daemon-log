...
module Fluent
  class DetachProcessManager
    ...
    class Broker
      def initialize
      puts 'jinchen ----- plugin.rb : broker : initialize'
      end

      def engine
        puts 'jinchen ----- plugin.rb : broker : engine start'
        Engine
        puts 'jinchen ----- plugin.rb : broker : engine end'
      end
    end

    def initialize
      puts 'jinchen ----- plugin.rb : detach-process-manager : initialize start'
      require 'drb'
      DRb.start_service(create_drb_uri, Broker.new)
      @parent_uri = DRb.uri
      puts 'jinchen ----- plugin.rb : detach-process-manager : initialize end'
    end

    def fork(forward_interval, delegate_object)
      puts 'jinchen ----- plugin.rb : detach-process-manager : fork start'
      ...
      puts 'jinchen ----- plugin.rb : detach-process-manager : fork end'
      return nil, forward_thread
    end

    private
    def read_header(ipr)
      puts 'jinchen ----- plugin.rb : detach-process-manager : read_header start'
      sz = ipr.read(4).unpack('N')[0]
      ipr.read(sz)
      puts 'jinchen ----- plugin.rb : detach-process-manager : read_header end'
    end

    def send_header(ipw, data)
      puts 'jinchen ----- plugin.rb : detach-process-manager : send_header start'
      ipw.write [data.bytesize].pack('N')
      ipw.write data
      ipw.flush
      puts 'jinchen ----- plugin.rb : detach-process-manager : send_header end'
    end

    def create_drb_uri
      puts 'jinchen ----- plugin.rb : detach-process-manager : create_drb_uri'
      "drbunix:"  # TODO
    end

    private
    def process_child(ipw, opr, forward_interval, delegate_object)
      puts 'jinchen ----- plugin.rb : detach-process-manager : process_child'
    ...
    def override_shared_methods(parent_uri)
      puts 'jinchen ----- plugin.rb : detach-process-manager : override_shared_methods'
    ...
    def shared_methods
      puts 'jinchen ----- plugin.rb : detach-process-manager : shared_methods'
      [
        #[:engine, Engine, :flush!],
        #[:engine, Engine, :stop],
      ]
    ...
    def process_parent(ipr, opw, pid, forward_interval, delegate_object)
      puts 'jinchen ----- plugin.rb : detach-process-manager : process_parent'
    ...
    def output_forward_main(opr, target)
      puts 'jinchen ----- plugin.rb : detach-process-manager : output_forward_main'
    ...
    def input_forward_main(ipr, pid)
      puts 'jinchen ----- plugin.rb : detach-process-manager : input_forward_main'
    ...
    def read_event_stream(r, &block)
      puts 'jinchen ----- plugin.rb : detach-process-manager : read_event_stream'
    ...
    def new_forwarder(w, interval)
      puts 'jinchen ----- plugin.rb : detach-process-manager : new_forwarder'
    ...
    class Forwarder
      def initialize(w)
        puts 'jinchen ----- plugin.rb : forwarder : initialize'
        @w = w
      end

      def emit(tag, es)
        puts 'jinchen ----- plugin.rb : forwarder : emit'
    ...
    class DelayedForwarder
      def initialize(w, interval)
        puts 'jinchen ----- plugin.rb : delayed-forwarder : initialize'
        @w = w
        @interval = interval
        @buffer = {}
        @mutex = Mutex.new
      ...
      def emit(tag, es)
        puts 'jinchen ----- plugin.rb : delayed-forwarder : emit'
      ...
      def run
        puts 'jinchen ----- plugin.rb : delayed-forwarder : run'
        while true
          sleep @interval
    ...
    class MultiForwarder
      def initialize(forwarders)
        puts 'jinchen ----- plugin.rb : multi-forwarder : initialize'
        @forwarders = forwarders
        @rr = 1
      end

      def emit(tag, es)
        puts 'jinchen ----- plugin.rb : multi-forwarder : emit'
  ...
  module DetachProcessImpl
    ...
    def detach_process_impl(num, forward_interval, &block)
      puts 'jinchen ----- process.rb :  detach_process_impl'
      children = []
    ...
    class FinishWait
      def initialize
        puts 'jinchen ----- process.rb :  finish-wait : initialize'
        @finished = false
        @mutex = Mutex.new
      ...
      def wait
        puts 'jinchen ----- process.rb :  finish-wait : wait'
        @mutex.synchronize do
          until @finished
            @cond.wait(@mutex, 1.0)
          end
      ...
      def stop
        puts 'jinchen ----- process.rb :  finish-wait : stop'
        return if @finished
        @finished = true
      ...
      def finished?
        puts 'jinchen ----- process.rb :  finish-wait : finished?'
        @finished
      end
    end
  end
  ...
  module DetachProcessMixin
    ...
    def configure(conf)
      puts 'jinchen ----- process.rb : configure'
      super
    ...
    def detach_process(&block)
      puts 'jinchen ----- process.rb : detach_process'
      if @detach_process
  ...
  module DetachMultiProcessMixin
    ...
    def detach_multi_process(&block)
      puts 'jinchen ----- process.rb : detach_multo_process'
      if @detach_process
