...
module Fluent
  class HttpInput < Input
    ...
    def initialize
      puts 'jinchen ----- in_http : httpinput : initialize'
    ...
    def configure(conf)
      puts 'jinchen ----- in_http : httpinput : configure'
      super
    ...
    class KeepaliveManager < Coolio::TimerWatcher
      def initialize(timeout)
        puts 'jinchen ----- in_http : keep-alive-manager : initialize'
        super(1, true)
        @cons = {}
      ...
      def add(sock)
        puts 'jinchen ----- in_http : keep-alive-manager : add'
        @cons[sock] = sock
      end

      def delete(sock)
        puts 'jinchen ----- in_http : keep-alive-manager : delete'
        @cons.delete(sock)
      end

      def on_timer
        #puts 'jinchen ----- in_http : keep-alive-manager : on_timer'
    ...
    def start
      puts 'jinchen ----- in_http : httpinput : start'
    ...
    def shutdown
      puts 'jinchen ----- in_http : httpinput : shutdown'
    ...
    def run
      puts 'jinchen ----- in_http : httpinput : run'
    ...
    def on_request(path_info, params)
      puts 'jinchen ----- in_http : httpinput : on_request'
      begin
    ...
    def parse_params_default(params)
      puts 'jinchen ----- in_http : httpinput : parse_params_default'
    ...
    def parse_params_with_parser(params)
      puts 'jinchen ----- in_http : httpinput : parse_params_with-parser'
    ...
    class Handler < Coolio::Socket
      def initialize(io, km, callback, body_size_limit, format, log, cors_allow_origins)
        puts 'jinchen ----- in_http : handler : initialize'
        super(io)
        @km = km
        @callback = callback
      ...
      def step_idle
        puts 'jinchen ----- in_http : handler : step_idle'
        @idle += 1
      end

      def on_close
        puts 'jinchen ----- in_http : handler : on_close'
        @km.delete(self)
      end

      def on_connect
        puts 'jinchen ----- in_http : handler : on_connect'
        @parser = Http::Parser.new(self)
      end

      def on_read(data)
        puts 'jinchen ----- in_http : handler : on_read'
        @idle = 0
        @parser << data
      rescue
      ...
      def on_message_begin
        puts 'jinchen ----- in_http : handler : on_message_begin'
        @body = ''
      end

      def on_headers_complete(headers)
        puts 'jinchen ----- in_http : handler : on_headers_complete'
        expect = nil
        size = nil
      ...
      def on_body(chunk)
        puts 'jinchen ----- in_http : handler : on_body'
      ...
      def on_message_complete
        puts 'jinchen ----- in_http : handler : on_message_conplete'
        return if closing?
      ...
      def on_write_complete
        puts 'jinchen ----- in_http : handler : on_write_complete'
        close if @next_close
      end

      def send_response_and_close(code, header, body)
        puts 'jinchen ----- in_http : handler : send_response_and_close'
        send_response(code, header, body)
        @next_close = true
      end
      ...
      def send_response(code, header, body)
        puts 'jinchen ----- in_http : handler : send_response'
      ...
      def send_response_nobody(code, header)
        puts 'jinchen ----- in_http : handler : send_response_nobody'
        data = %[HTTP/1.1 #{code}\r\n]
        header.each_pair {|k,v|
          data << "#{k}: #{v}\r\n"
        }
