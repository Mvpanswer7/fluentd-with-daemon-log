...
module Fluent
  class OutputChain
    def initialize(array, tag, es, chain=NullOutputChain.instance)
      puts 'jinchen ----- output.rb : output-chain : initialize'
      @array = array
      @tag = tag
      @es = es
      @offset = 0
    ...
    def next
      puts 'jinchen ----- output.rb : output-chain : next'
  ...
  class CopyOutputChain < OutputChain
    def next
      puts 'jinchen ----- output.rb : copy-output-chain : next'
      if @array.length <= @offset
        return @chain.next
      end
      @offset += 1
  ...
  class NullOutputChain
    ...
    def next
      puts 'jinchen ----- output.rb : null-output-chain : next'
    end
  end
  ...
  class Output
    ...
    def initialize
      puts 'jinchen ----- output.rb : output : initialize'
      super
    end

    def configure(conf)
      puts 'jinchen ----- output.rb : output : configure'
      super
    ...
    def start
      puts 'jinchen ----- output.rb : output : start'
    end

    def shutdown
      puts 'jinchen ----- output.rb : output : shutdown'
    end
    ...
    def secondary_init(primary)
      puts 'jinchen ----- output.rb : output : secondary_init'
  ...
  class OutputThread
    def initialize(output)
      puts 'jinchen ----- output.rb : output-thread : initialize'
      @output = output
      @finish = false
    ...
    def configure(conf)
      puts 'jinchen ----- output.rb : output-thread : configure'
    end

    def start
      puts 'jinchen ----- output.rb : output-thread : start'
      @mutex = Mutex.new
    ...
    def shutdown
      puts 'jinchen ----- output.rb : output-thread : shutdown'
      @finish = true
      @mutex.synchronize {
        @cond.signal
      }
    ...
    def submit_flush
      puts 'jinchen ----- output.rb : output-thread : submit_flush'
      @mutex.synchronize {
        @next_time = 0
        @cond.signal
      }
    ...
    private
    def run
      puts 'jinchen ----- output.rb : output-thread : run'
      @mutex.lock
      begin
        until @finish
          time = Time.now.to_f
    ...
    def cond_wait(sec)
      puts 'jinchen ----- output.rb : output-thread : cond_wait'
      @cond.wait(@mutex, sec)
    end
  end
  ...
  class BufferedOutput < Output
    def initialize
      puts 'jinchen ----- output.rb : buffered-output : initialize'
      super
      @next_flush_time = 0
      @last_retry_time = 0
      @next_retry_time = 0
    ...
    def configure(conf)
      puts 'jinchen ----- output.rb : buffered-output : configure'
      super
    ...
    def start
      puts 'jinchen ----- output.rb : buffered-output : start'
    ...
    def shutdown
      puts 'jinchen ----- output.rb : buffered-output : shutdown'
    ...
    def emit(tag, es, chain, key="")
      puts 'jinchen ----- output.rb : buffered-output : emit'
      @emit_count += 1
    ...
    def submit_flush
      puts 'jinchen ----- output.rb : buffered-output : submit_flush'
    ...
    def format_stream(tag, es)
      puts 'jinchen ----- output.rb : buffered-output : format_stream'
      out = ''
      es.each {|time,record|
    ...
    def enqueue_buffer(force = false)
      puts 'jinchen ----- output.rb : buffered-output : enqueue_buffer'
    ...
    def try_flush
      puts 'jinchen ----- output.rb : buffered-output : try_flush'
      time = Time.now.to_f
    ...
    def force_flush
      puts 'jinchen ----- output.rb : buffered-output : force_flush'
      @num_errors_lock.synchronize do
    ...
    def before_shutdown
      puts 'jinchen ----- output.rb : buffered-output : before_shutdown'
      begin
    ...
    def calc_retry_wait
      puts 'jinchen ----- output.rb : buffered-output : calc_retry_wait'
      # TODO retry pattern
    ...
    def write_abort
      puts 'jinchen ----- output.rb : buffered-output : write_abort'
    ...
    def flush_secondary(secondary)
      puts 'jinchen ----- output.rb : buffered-output : flush_secondary'
  ...
  class ObjectBufferedOutput < BufferedOutput
    def initialize
      puts 'jinchen ----- output.rb : object-buffered-output : initialize'
      super
    end

    def emit(tag, es, chain)
      puts 'jinchen ----- output.rb : object-buffered-output : emit'
      @emit_count += 1
    ...
    module BufferedEventStreamMixin
      ...
      def repeatable?
        puts 'jinchen ----- output.rb : buffered-event-output : repeatable?'
        true
      end

      def each(&block)
        puts 'jinchen ----- output.rb : buffered-event-output : each'
        msgpack_each(&block)
      end

      def to_msgpack_stream
        puts 'jinchen ----- output.rb : buffered-event-output : to_msgpack_stream'
        read
      end
    end

    def write(chunk)
      puts 'jinchen ----- output.rb : buffered-event-output : write'
  ...
  class TimeSlicedOutput < BufferedOutput
    ...
    def initialize
      puts 'jinchen ----- output.rb : time-sliced-output : initialize'
      super
      @localtime = true
    ...
    def configure(conf)
      puts 'jinchen ----- output.rb : time-sliced-output : configure'
      super
    ...
    def emit(tag, es, chain)
      puts 'jinchen ----- output.rb : time-sliced-output : emit'
      @emit_count += 1
      formatted_data = {}
      es.each {|time,record|
        begin
    ...
    def enqueue_buffer(force = false)
      puts 'jinchen ----- output.rb : time-sliced-output : enqueue_buffer'
      if force
        @buffer.keys.each {|key|
          @buffer.push(key)
        }
    ...
    def time_slice_cache_interval
      puts 'jinchen ----- output.rb : time-sliced-output : time_slice_cache_interval'
