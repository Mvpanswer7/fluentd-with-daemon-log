...
module Fluent
  ...
  class RootAgent < Agent
    ...
    def initialize(system_config = SystemConfig.new)
      puts 'jinchen ----- root_agent.rb : root-agent : initialize'
      super
    ...
    def configure(conf)
      puts 'jinchen ----- root_agent.rb : root-agent : configure'
    ...
    def setup_error_label(e)
      puts 'jinchen ----- root_agent.rb : root-agent : setup_error_label'
    ...
    def start
      puts 'jinchen ----- root_agent.rb : root-agent : start'
      super
    ...
    def shutdown
      puts 'jinchen ----- root_agent.rb : root-agent : shutdown'
      # Shutdown Input plugin first to prevent emitting to terminated Output plugin
      @started_inputs.map { |i|
        Thread.new do
          begin
            log.info "shutting down input", type: Plugin.lookup_name_from_class(i.class), pluginjinchen_id: i.plugin_id
            i.shutdown
          rescue => e
    ...
    def suppress_interval(interval_time)
      puts 'jinchen ----- root_agent.rb : root-agent : suppress_interval'
      @suppress_emit_error_log_interval = interval_time
      @next_emit_error_log_time = Time.now.to_i
    end

    def add_source(type, conf)
      puts 'jinchen ----- root_agent.rb : root-agent : add_source'
    ...
    def add_label(name)
      puts 'jinchen ----- root_agent.rb : root-agent : add_label'
    ...
    def find_label(label_name)
      puts 'jinchen ----- root_agent.rb : root-agent : find_lable'
    ...
    def emit_error_event(tag, time, record, error)
      puts 'jinchen ----- root_agent.rb : root-agent : emit_error_event'
    ...
    def handle_emits_error(tag, es, error)
      puts 'jinchen ----- root_agent.rb : root-agent : handle_emit_error'
    ...
    class RootAgentProxyWithoutErrorCollector < SimpleDelegator
      def initialize(root_agent)
        puts 'jinchen ----- root_agent.rb : root-agent-proxy-without-error-collector : initialize'
        super
      ...
      def emit_error_event(tag, time, record, error)
        puts 'jinchen ----- root_agent.rb : root-agent-proxy-without-error-collector : emit_error_event'
      ...
      def handle_emits_error(tag, es, e)
        puts 'jinchen ----- root_agent.rb : root-agent-proxy-without-error-collector : handle_emits_error'
        now = Engine.now
        if @suppress_emit_error_log_interval.zero? || now > @next_emit_error_log_time
