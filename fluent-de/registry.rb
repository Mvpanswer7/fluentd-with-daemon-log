...
puts 'jinchen ### registry.rb'
require 'fluent/config/error'

module Fluent
  class Registry
    def initialize(kind, search_prefix)
      puts 'jinchen ----- registry.rb : registry : initialize'
      @kind = kind
    ...
    def register(type, value)
      puts 'jinchen ----- registry.rb : registry : register'
      type = type.to_sym
      puts type 
      puts value
      @map[type] = value
    ...
    def lookup(type)
      puts 'jinchen ----- registry.rb : registry : lookup'
      type = type.to_sym
      if value = @map[type]
        return value
      end
    ...
    def search(type)
      puts 'jinchen ----- registry.rb : registry : search'
