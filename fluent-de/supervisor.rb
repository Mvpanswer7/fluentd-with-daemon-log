...
module Fluent
  class Supervisor
    def self.get_etc_passwd(user)
      puts 'jinchen ----- supervisor.rb : self.get_etc_passwd'
    ...
    def self.get_etc_group(group)
      puts 'jinchen ----- supervisor.rb : self.get_etc_group'
    ...
    class LoggerInitializer
      ...
      puts 'jinchen ----- supervisor.rb : logger-initializer : initialize'
        @path = path
        @level = level
        @chuser = chuser
        @chgroup = chgroup
      ...
      def init
      puts 'jinchen ----- supervisor.rb : logger-initializer : init'
        if @path && @path != "-"
          @io = File.open(@path, "a")
          if @chuser || @chgroup
      ...
      def stdout?
      puts 'jinchen ----- supervisor.rb : logger-initializer : stdout?'
        @io == STDOUT
      end

      def reopen!
      puts 'jinchen ----- supervisor.rb : logger-initializer : reopen!'
        if @path && @path != "-"
          @io.reopen(@path, "a")
        end
        self
      ...
      def level=(level)
      puts 'jinchen ----- supervisor.rb : logger-initializer : level='
        @level = level
        $log.level = level
      end
    end
    ...
    def initialize(opt)
      puts 'jinchen ----- supervisor.rb : initialize '
    ...
    def start
      puts 'jinchen ----- supervisor.rb : start'
      @log.init
    ...
    def options
      puts 'jinchen ----- supervisor.rb : options'
      {
    ...
    def dry_run
      puts 'jinchen ----- supervisor.rb : dry_run'
    ...
    def show_plugin_config
      puts 'jinchen ----- supervisor.rb : show_plugin_config'
    ...
    def start_daemonize
      puts 'jinchen ----- supervisor.rb : start_daemonize'
    ...
    def wait_daemonize
      puts 'jinchen ----- supervisor.rb : wait_daemonize'
    ...
    def finish_daemonize
      puts 'jinchen ----- supervisor.rb : finish_daemonize'
    ...
    def setup_rpc_server
      puts 'jinchen ----- supervisor.rb : setup_rpc_server'
    ...
    def setup_rpc_get_dump
      puts 'jinchen ----- supervisor.rb : setup_rpc_get_dump'
    ...
    def run_rpc_server
      puts 'jinchen ----- supervisor.rb : run_rpc_server'
      @rpc_server.start
    end

    def stop_rpc_server
      puts 'jinchen ----- supervisor.rb : stop_rpc_server'
      @rpc_server.shutdown
    end

    def supervise(&block)
      puts 'jinchen ----- supervisor.rb : supervise'
      start_time = Time.now
    ...
    def main_process(&block)
      puts 'jinchen ----- supervisor.rb : main_process'
    ...
    def install_supervisor_signal_handlers
      puts 'jinchen ----- supervisor.rb : install_supervisor_signal_handlers'
      trap :INT do
    ...
    def supervisor_sigint_handler
      puts 'jinchen ----- supervisor.rb : supervisor_sigint_handler'
      @finished = true
      if pid = @main_pid
    ...
    def supervisor_sigterm_handler
      puts 'jinchen ----- supervisor.rb : supervisor_sigterm_handler'
      @finished = true
      if pid = @main_pid
        # kill processes only still exists
        unless Process.waitpid(pid, Process::WNOHANG)
    ...
    def supervisor_sighup_handler
      puts 'jinchen ----- supervisor.rb : supervisor_sighup_handler'
    ...
    def supervisor_sigusr1_handler
      puts 'jinchen ----- supervisor.rb : supervisor_sigusr1_handler'
      @log.reopen!
      if pid = @main_pid
    ...
    def supervisor_dump_config_handler
      puts 'jinchen ----- supervisor.rb : supervisor_dump_config_handler'
      $log.info @conf.to_s
    end

    def supervisor_get_dump_config_handler
      puts 'jinchen ----- supervisor.rb : supervisor_get_dump_config_handler'
      {conf: @conf.to_s}
    end

    def read_config
      puts 'jinchen ----- supervisor.rb : read_config'
    ...
    def set_system_config
      puts 'jinchen ----- supervisor.rb : set_system_config'
      @system_config = SystemConfig.create(@conf) # @conf is set in read_config
      @system_config.apply(self)
    end

    def run_configure
      puts 'jinchen ----- supervisor.rb : run_configure'
      Fluent::Engine.run_configure(@conf)
    end

    def change_privilege
      puts 'jinchen ----- supervisor.rb : change_privilege'
      if @chgroup
    ...
    def init_engine
      puts 'jinchen ----- supervisor.rb : init_engine'
    ...
    def install_main_process_signal_handlers
      puts 'jinchen ----- supervisor.rb : install_main_process_signal_handlers'
    ...
    def run_engine
      puts 'jinchen ----- supervisor.rb : run_engine'
      Fluent::Engine.run
    end
  end
end
